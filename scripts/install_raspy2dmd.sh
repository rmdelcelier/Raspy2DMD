...
step_install_rgbmatrix() {
    ...
    # Le repo rpi-rgb-led-matrix a migre vers scikit-build-core + cmake.
    # Le pyproject.toml est a la RACINE du repo (pas dans bindings/python/).
    # Les targets 'make build-python' / 'make install-python' ont ete supprimees.
    # => Installer avec : pip3 install --break-system-packages .  (depuis la racine)

    # Prerequis : cmake et scikit-build-core
    log_substep "Installation de cmake et scikit-build-core..."
    apt-get install -y -qq cmake ninja-build >> "$LOG_FILE" 2>&1
    pip3 install --break-system-packages scikit-build-core >> "$LOG_FILE" 2>&1

    # Sur ARMv6, utiliser python3.10 (PyArmor 7.x necessite Python <= 3.10)
    if [ "$IS_PI_ZERO_ARMv6" = "true" ] && command -v python3.10 &>/dev/null; then
        log_substep "Compilation et installation des bindings Python pour Python 3.10 (ARMv6)..."
        python3.10 -m pip install --break-system-packages . >> "$LOG_FILE" 2>&1
        ...
        log_substep "Compilation et installation des bindings Python (peut prendre plusieurs minutes)..."
        pip3 install --break-system-packages . >> "$LOG_FILE" 2>&1
